...
public class AuxiliaryListener extends Utility implements Listener {
	...
	public void onSpawn(CreatureSpawnEvent e) {
		...
		if (conf.has(et)) {
			if (e.getEntity() instanceof Sheep) {
				...
				if (!sheep.isAdult()) {
					if (conf.block(et, Type.BABY, color)) {
						e.setCancelled(true);
						return;
					} else {
						return;
				...
				if (conf.block(et, color)) {
					e.setCancelled(true);
				}
				return;
			} else if (e.getEntity() instanceof Ageable) {
				Ageable a = (Ageable) e.getEntity();
				if (!a.isAdult()) {
					if (conf.block(et, Type.BABY)) {
						e.setCancelled(true);
					} else {
						return;
					}
				...
				Zombie z = (Zombie) e.getEntity();
				  if (z.isBaby() && !z.isVillager()) {
					...
					if (conf.block(et, Type.VILLAGER)) {					
						e.setCancelled(true);
					}
				} else if (z.isBaby() || z.isVillager()) {
					if (conf.block(et, Type.BOTH)) {						
						e.setCancelled(true);
					}
				}
				return;
	...
	public void onTrade(PlayerInteractEntityEvent e) {
		if (e.getRightClicked() instanceof org.bukkit.entity.Villager) {
			...
			String et = e.getRightClicked().getType().toString();
			...
			if (conf.has(et) && is.getTypeId() == 351) {	
				...
				if (!sheep.isAdult()) {
					if (conf.block(et, Type.BABY, dc.getWoolData())) {
						e.setCancelled(true);
						al(conf, p, "&cThat color of sheep is blocked");
						al(conf, "Player " + p.getName() + " tried to dye a sheep " + dc.toString().toLowerCase());
						update(sheep);
						return;
					} else {
						return;
				...
				if (conf.block(et, dc.getWoolData())) {
					e.setCancelled(true);
					al(conf, p, "&cThat color of sheep is blocked");
					al(conf, "Player " + p.getName() + " tried to dye a sheep " + dc.toString().toLowerCase());
					update(sheep);
	...
	public void onInteraction(PlayerInteractEvent e) {
		...
		if (e.getAction() == RIGHT_CLICK_AIR
			...
			} else if (c.get(2).contains(e.getItem().getType().toString())) {
